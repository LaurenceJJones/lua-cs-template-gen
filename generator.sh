...
install_tailwind_bin() {
    ...
    if [ ! -f "bin/tailwind" ]; then
        ...
        download "https://github.com/tailwindlabs/tailwindcss/releases/download/v3.2.7/tailwindcss-linux-x64" "bin"
...
inject_css_into_html() {
    ...
    echo "${FG_GREEN}You can now open dist/ban.html and dist/captcha.html in your browser${RESET}"
}

generate_dev_css() {
    echo "${FG_GREEN}Watching for changes to HTML${RESET}"
    echo "${FG_GREEN}You can now open src/ban.html and src/captcha.html in your browser${RESET}"
